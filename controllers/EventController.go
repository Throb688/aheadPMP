...
func (e EventController) SearchForEvent(c *gin.Context) {
	query := c.Query("q")
	name, expensesResult, incomeResult := services.SearchForEventData(query)
	utils.Success(c, map[string]interface{}{"code": int(utils.ApiCode.SUCCESS), "name": name, "expenses": expensesResult, "income": incomeResult})
	return
}
